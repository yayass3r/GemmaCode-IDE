...
export async function GET(request: NextRequest) {
  try {
    ...
    if (publishedFilter === 'true') where.isPublished = true
    else if (publishedFilter === 'false') where.isPublished = false
    ...
    return NextResponse.json({ error: 'Failed to fetch your projects' }, { status: 500 })
  }
}
